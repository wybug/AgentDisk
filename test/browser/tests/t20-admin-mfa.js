...
describe('T20: Admin MFA 通行密钥管理', () => {
  ...
  ab.screenshot('t20-02-switch-disabled');
  ...
  ab.screenshot('t20-03-cancel-registration');
  ...
  ab.screenshot('t20-04-passkey-registered');
  ...
  ab.screenshot('t20-05-switch-enabled');
  ...
  // ── TC-79: macOS 平台验证器对 (RPID, userId) 去重，无法注册第二个 passkey，已跳过 ──
  ...
  `);
  step('TC-80: 删除通行密钥', true, 'delete=' + deleteFirstResult + ' confirm=' + confirmDelete + ' remaining=' + afterFirstDelete);
  ab.screenshot('t20-07-after-delete');
  ...
  step('TC-81: 清理剩余通行密钥', true, 'deleted=' + remainingCreds.length);
  ...
  ab.screenshot('t20-08-cleanup-done');
