...
describe('T14: 测试网关管理', () => {
  ...
  assertCondition(dashLoaded, 'T14.1: 网关仪表盘加载成功');

  // T14.2 - 查看 OAuth2 配置区域
  const hasOAuth = ab.pageContainsText('Client') || ab.pageContainsText('OAuth2') || ab.pageContainsText('oauth');
  assertCondition(hasOAuth, 'T14.2: OAuth2 配置信息显示');
  ab.screenshot('t12-02-oauth-config');

  // T14.3 - 添加测试用户（使用直接 ID 选择器避免 placeholder 匹配冲突）
  ...
  step('T14.3: 测试用户添加', added, 'fill=' + addUserResult + ' click=' + addClick);
  ab.screenshot('t12-03-user-added');

  // T14.4 - 通过 API 删除测试用户（绕过 confirm() 阻塞问题）
  if (added) {
    ...
    step('T14.4: 测试用户删除', gone, delResult);
  } else {
    step('T14.4: 跳过删除（用户未添加成功）', false, '前置条件不满足');
  }
  ab.screenshot('t12-04-user-deleted');

  // T14.5 - 验证「打开 AgentDisk」链接
  const hasOpenLink = ab.pageContainsText('AgentDisk') || ab.pageContainsText('9101');
  step('T14.5: 页面包含 AgentDisk 链接信息', hasOpenLink);
  ab.screenshot('t12-05-agentdisk-link');

  // T14.6 - 访问 OAuth2 调试页面
  ...
  step('T14.6: OAuth2 调试页面加载', debugLoaded);
